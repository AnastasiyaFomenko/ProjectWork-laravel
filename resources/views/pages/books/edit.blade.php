            ...
            <div class="col-8 col-md-6 background-lilac-52">
                <div class="center">
                    ...
                    <form action="{{ route('books.update', ['book' => $book->id]) }}" method="post"
                        ...
                        <x-text-input id="name_id" type="text" name="name_id" required autofocus value="{{ $book->name_id  }}" />
                        <x-input-error :messages="$errors->get('name_id')" />

                        <x-input-label for="age_limit_id" :value="__('Id возрастного ограничения')" />
                        <x-text-input id="age_limit_id" type="text" name="age_limit_id" required value="{{ $book->age_id  }}"/>
                        <x-input-error :messages="$errors->get('age_limit_id')" />

                        <x-input-label for="annotation_id" :value="__('Id аннотация')" />
                        <x-text-input id="annotation_id" type="text" name="annotation_id" required value="{{ $book->annotation_id  }}"/>
                        ...
                        <x-text-input id="house_id" type="text" name="house_id" required value="{{ $book->house_id  }}"/>
                        <x-input-error :messages="$errors->get('house_id')" />

                        <x-input-label for="language_id" :value="__('Id языка книги')" />
                        <x-text-input id="language_id" type="text" name="language_id" required value="{{ $book->language_id  }}"/>
                        ...
                        <x-text-input id="binding_id" type="text" name="binding_id" required value="{{ $book->binding_id  }}"/>
